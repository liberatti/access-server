...
class SQLiteDAO:
    ...
    def persist(self, vo):
        ...
        if pk_attr:
            if pk_attr not in vo or len(vo[pk_attr]) <= 0:
                vo.update({self.__PK__: self.gen_id()})

        columns = ", ".join(vo.keys())
        vals = ", ".join(f":{key}" for key in vo.keys())
        query = f"INSERT INTO {self.__collection_name__} ({columns}) VALUES ({vals})"
        logger.debug(f"{query} : {str(vo)}")

        try:
            cursor = self.__connection.cursor()
            cursor.execute(query, vo)
            if getattr(self, "__PK__", False):
                return vo[self.__PK__]
        finally:
            cursor.close()
    ...
    def update_by_id(self, pk, vo):
        vals = ", ".join(f"{key} = :{key}" for key in vo.keys())
