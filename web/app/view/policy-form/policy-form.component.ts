...
export class PolicyFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.userService.get().subscribe((data: any) => {
      this.__clients = data?.data || (Array.isArray(data) ? data : []);
    });
  ...
  onAddClient(): void {
    ...
    if (this.form.value.clients != null) {
      const cExists = this.form.value.clients.some(c1 => c1.id === data.id);
      if (!cExists) {
  ...
  onAddNetwork(): void {
    ...
    if (this.form.value.networks != null) {
      const nExists = this.form.value.networks.includes(data);
      if (!nExists)
